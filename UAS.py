# First we import the  OpenCV module
import cv2
#Now we import the numpy module and call it np
import numpy as np

#Then the image is loaded
img = cv2.imread('sample_image.jpeg')

#After loading the image is converted to HSV
hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)

#We need to define the lower bounds and upper bounds for green and brown region
lower_brown = np.array([10, 50, 50])
upper_brown = np.array([20, 255, 255])
lower_green = np.array([40, 50, 50])
upper_green = np.array([70, 255, 255])

#A mash is to be created for both the regions
mask_brown = cv2.inRange(hsv, lower_brown, upper_brown)
mask_green = cv2.inRange(hsv, lower_green, upper_green)

#The contours have to be found in the masks
